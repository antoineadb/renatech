dojo.provide("dojox.charting.Pie3d");
...
dojo.declare("dojox.gfx3d.Sector", dojox.gfx3d.Object,{
	...
	constructor: function() {
		...
		this.object = dojo.clone({type: "sector", 
			rx: 100, 
			ry: 50, 
			depth: 30, 
			normal: null, 
	...
	render: function(camera){
		...
		var marks = dojo.map([this.object.begin, this.object.end], function(item){
			return {x: this.rx* Math.cos(item), 
				y: this.ry* Math.sin(-item), z: 0};
			}, this.object);
        this.cache = {begin: marks[0], end: marks[1]};
	},
	...
	inView: function(begin, end){
		if(end > Math.PI){    //Bottom half of Pie chart radians begin right and counterclockwise 2Pie in a circle
			var s = begin;      //so we are dealing with the bottom half    
			var e = end;
			if(begin <= Math.PI){
				s = Math.PI;  //its fine to this point
			}
			if(end > Math.PI * 2){
				e = Math.PI * 2; //cut to the end of the circle do not call around again
			}
			return [s, e];
	...
	draw: function(lighting){
		...
		var gradient={'type': 'linear','x1': 0,'x2': 0,'y2': 0,'y1': 100,
		 'colors': [{offset: 0, color: work},{offset: 1, color: body}]};  
		...
		if (diff>=Math.PI){
			...
			var A = {begin: half[0], end: half[1]};
	
			var other = dojo.map([this.object.begin+diff/2.0, this.object.end], function(item) {
				return {x: this.rx* Math.cos(item), 
					y: this.ry* Math.sin(-item), z: 0};
				}, this.object);
			var B = {begin: other[0], end: other[1]};
	
	
			//fill
			this.shape.createPath("")
				.moveTo(0, 0)
				.lineTo(A.begin.x, A.begin.y)
				.arcTo(this.object.rx, this.object.ry, 0, false, false,  A.end)
				.lineTo(0,0)
				.setStroke(null) //this.strokeStyle)
				.setFill((this.object.shade===true)?gradient:this.toStdFill(lighting, this.object.normal));
		 
			//Arc line
			this.shape.createPath("")
				.moveTo(0, 0)
				.moveTo(A.begin.x, A.begin.y)
				.arcTo(this.object.rx, this.object.ry, 0, false, false,  A.end)
			...
			this.shape.createPath("")
				.moveTo(0, 0)
				.lineTo(B.begin.x, B.begin.y)
				.arcTo(this.object.rx, this.object.ry, 0, false, false,  B.end)
				.setStroke(null)
				.setFill((this.object.shade===true)?gradient:this.toStdFill(lighting, this.object.normal));
			...
			this.shape.createPath("")
				.moveTo(0, 0)
				.moveTo(B.begin.x, B.begin.y)
				.arcTo(this.object.rx, this.object.ry, 0, false, false,  B.end)
				.setStroke(this.strokeStyle);
		}else{
			this.shape.createPath("")
				.moveTo(0, 0)
				.lineTo(c.begin.x, c.begin.y)
				.arcTo(this.object.rx, this.object.ry, 0, false, false,  c.end)
				.setStroke(this.strokeStyle)
				.setFill((this.object.shade===true)?gradient:this.toStdFill(lighting, this.object.normal));
		}
		...
		if(!p){
			return;  //if its not visible the cylinder part is not drawn
		}
		...
		var surf = this.shape.createPath("")
			.moveTo(s.x, s.y)
			.arcTo(this.object.rx, this.object.ry, 0, false, false, e)
			.lineTo(e.x, e.y + this.object.depth)
			.arcTo(this.object.rx, this.object.ry, 0, false, true, 
				{x: s.x, y: s.y + this.object.depth})
			.lineTo(s.x, s.y)
			.setFill(darker)
			.setStroke(this.strokeStyle);
	} 
...
dojo.declare("dojox.charting.Pie3d",null,{

	colorIndex: 0,

	//default colors when use the 'default' word
	colors:['#FF9090','#90FF90','#9090FF','#B9D3EE','#FFD39B',
			'#70DB93','#F4A460','#8B7355','#FFF68F','#BA55D3',
			'#FFFF00','#236B8E','#EEA9B8','#FF2400','#F0FFF0'],
	...
	constructor: function(inid,opt) {
		...
		this.dividDom = dojo.query("#" + inid)[0];
	...
	addSeries: function(series){
		...
		if(series.fillStyle.color === "default"){
			series.fillStyle.color= this.colors[this.colorIndex];
			++this.colorIndex;
	...
	render: function() {
		...
		var normal = {x: Math.sin(theta),  y: 0, z: Math.cos(theta)};

		var sum = 0;
		dojo.forEach(this.series, function(item){
			sum += item.data;        //total data values
		});
		...
		dojo.forEach(this.series, function(item){
			...
			item.cangle = beginAngle+angle/2.0;      //center angle
			beginAngle += angle;
			sector.end   = beginAngle;
			sector.normal= normal;

			//you could make stroke an option
			item.sector = this.view.create3DObject(dojox.gfx3d.Sector, sector)
				.setFill(item.fillStyle)
				.setStroke('#505050'); //save sectors
		}, this);
		...
		var hh = this.spec.ry* Math.cos(Math.PI/8)+this.spec.depth+topmargin;  //actual bottom of Pie from center
		var tx = this.dimensions.width/2;
		var ty = hh; //this.dimensions.width/2;

		dojo.forEach(this.series, function(item){
			var x=self.spec.rx* Math.cos(item.cangle)+tx-10; //the -10 is a nice shift for label placement
			var y=self.spec.ry* Math.sin(-item.cangle)+ty-10;
			self.surface.createText({x: x, y: y, text: item.title})
				.setFont({weight: "bold", family: "arial", size: "8pt"})
				.setFill("black");
		...
		var modulo= ((this.series.length/2)+1)|0; //two columns only
		var fontH;
		try{
		...
		if(this.label !== null){
			this.surface.createText({x: tx, y: y, align: 'middle', text: this.label})
				.setFont({weight: "bold", family: "arial", size: "8pt"})
				.setFill(this.lcolor);
		}
		dojo.forEach(this.series, function(item){
			...
			++i;
			self.surface.createText({x: x+5, y: y, text: item.title})
				.setFont({weight: "normal", family: "arial", size: "8pt"})
				.setFill("black");

			self.surface.createRect({
				x: x-boxwidth,
				y: y-fontH/2,
				width: 10, height: fontH/2
