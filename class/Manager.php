 ...
 /**
    ...
    public function addParams(Params $param) {
        try {
            ...
            $requete = $this->_db->prepare('INSERT INTO params (description,login,mdp,host)VALUES(?,?,?,?)');
            $login = $param->getLogin();
            $mdp = $param->getMdp();
            $host = $param->getHost();
            $port = $param->getPort();            
            $requete->bindParam(1, $login, PDO::PARAM_STR);
            $requete->bindParam(2, $mdp, PDO::PARAM_STR);
            $requete->bindParam(3, $host, PDO::PARAM_STR);
            $requete->bindParam(4, $port, PDO::PARAM_STR);
