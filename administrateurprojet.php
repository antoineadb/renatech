...
<div id="global">
    ...
    <div style="margin-top:15px;width:1050px" id="navigation">
        <form  method="post" action="<?php echo '/' . REPERTOIRE ?>/projet_admin/<?php echo $lang; ?>" id='gestionporteur' name='gestionporteur' >
            ...
            <fieldset id="new" style="border-color: #5D8BA2;width: 1050px;padding:4px; margin-top: 15px" >
                ...
                <?php include 'findProjetadmin.php';?>
            </fieldset><?php
            }
            ?>
<?php if (!empty($_GET['messagedejaaffecte'])) { ?>
                <br>
...
<?php if (!empty($_GET['idutilisateur'])) { ?>
            ...
            if (isset($_GET['idutilisateur'])) {
                $row = $manager->getList2("SELECT dateaffectation,numero,titre,ua.idprojet FROM utilisateuradministrateur ua, projet p WHERE  p.idprojet = ua.idprojet and datestatutcloturer is null and ua.idutilisateur=?", $_GET['idutilisateur']);
                $nom = $manager->getSingle2("select nom from utilisateur where idutilisateur = ?", $_GET['idutilisateur']);
                ?><br>
                <fieldset id="findUseradmin" style="border-color: #5D8BA2;width: 1040px;padding-bottom:10px;padding-top: 5px;border:solid 1px #5D8BA2;color:midnightblue;font-family:verdana;border-radius: 5px;" >
                    <legend style="margin-left: 20px"><?php echo TXT_PROJETADMINISTREPAR . ucfirst($nom); ?></legend>

                    <?php
                    $fprow = fopen('tmp/adminProjet.json', 'w');
                    $dataadminprojet = "";
                    fwrite($fprow, '{"items": [');
                    for ($i = 0; $i < count($row); $i++) {
                        $dataadminprojet = "" . '{"dateaffectation":' . '"' . $row[$i]['dateaffectation'] . '"' . "," . '"numero":' . '"' . $row[$i]['numero'] . '"' .
                                "," . '"titre":' . '"' . filtredonnee($row[$i]['titre']) . '"' .
                                "," . '"idprojet":' . '"' . $row[$i]['idprojet'] . '"' . "," .
                                '"Enlever":' . '"' . TXT_RETIRER . '"' . "},";
                        fputs($fprow, $dataadminprojet);
                        fwrite($fprow, '');
                    }
                    fwrite($fprow, ']}');
                    $json_fileadminprojet = "tmp/adminProjet.json";
                    $jsonadminprojet1 = file_get_contents($json_fileadminprojet);
                    $jsonAdminprojet = str_replace('},]}', '}]}', $jsonadminprojet1);
                    file_put_contents($json_fileadminprojet, $jsonAdminprojet);
                    fclose($fprow);
                    chmod('tmp/adminProjet.json', 0777);
                    if (count($row) == 0) {
                    ...
                    <div style="height: 250px;">
                        <div id="griduseradmin" ></div>
                        <script>
                            var griduseradmin, dataStore, store;
                            require([
                                "dojox/grid/DataGrid",
                                "dojo/store/Memory",
                                ...
                                request.get("<?php echo '/' . REPERTOIRE; ?>/tmp/adminProjet.json", {
                                    handleAs: "json"
                                }).then(function(data) {
                                    ...
                                    griduseradmin.startup();
                                    griduseradmin.on("RowClick", function(evt) {
                                        var idx = evt.rowIndex;
                                        var item = griduseradmin.getItem(idx);
                    ...
                    <script>
                        function retireprojet(idprojet) {
                            window.location.replace('<?php echo "/" . REPERTOIRE ?>/modifBase/deleteutilisateuradminprojet.php?lang=<?php echo $lang; ?>&idprojet=' + idprojet + '&idutilisateur=<?php echo $_GET['idutilisateur']; ?>');
                        }
                    </script>
            ...
            if (isset($_GET['idprojet']) && !empty($_GET['idprojet'])) {
                ...
                <div id='fieldprojetporteur'  >
                    <table style="border: 1px solid #57758D;clear: both;margin-top: 15px;width: 1050px;border-radius: 5px;">
                        <th style="font-size:1.2em;background-color: #E3ECF2; padding-left:6px; color: midnightblue;height:20px" ><?php echo TXT_SELECTEDPROJET; ?></th>
                        <tr>
                            <td>                                
                    ...
                    <table><tr>  <td><button type="button" data-dojo-type="dijit/form/Button"  id="annule" data-dojo-props="onClick:function(){window.location.replace('<?php echo "/" . REPERTOIRE ?>/recherche_projet_admin/<?php echo $lang; ?>')}"><?php echo TXT_ANNULE ?></button></td></tr></table>
                    <br>
                    <fieldset id="ident" style="border-color: #5D8BA2;width: 1008px;padding-bottom:10px;padding-top:8px;margin-top:20px" >
                        ...
                        <table>
                            <tr>
                                ...
                                <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
                                <td>&nbsp;&nbsp</td>
                                <td valign="bottom" ><input name ="recchercheUseradmin" type="submit" ondragover="tous"  value="<?php echo TXT_BOUTONRECHERCHE; ?>"  class="maj"
