...
if (isset($_POST['numprojet']) && !empty($_POST['numprojet']) && $_POST['numprojet'] != "*" && isset($_POST['dateprojet']) && !empty($_POST['dateprojet'])) {
    $req = "SELECT idprojet, titre, datedebutprojet, numero, dateprojet FROM concerne, projet WHERE idprojet_projet = idprojet and lower(numero) like lower(?) and idstatutprojet_statutprojet=? and dateprojet=? and idcentrale_centrale=? AND trashed =FALSE"
            . "union"
            . "SELECT idprojet, titre, datedebutprojet, numero, dateprojet FROM concerne, projet WHERE idprojet_projet = idprojet and lower(numero) like lower(?) and idstatutprojet_statutprojet=? and dateprojet=? and idcentrale_centrale=? AND trashed =FALSE";
    $param = array('%' . $numprojet . '%', ENCOURSREALISATION, $dateprojet, $idcentrale, '%' . $numprojet . '%', ACCEPTE, $dateprojet, $idcentrale);
}elseif (isset($_POST['dateprojet']) && !empty($_POST['dateprojet'])&& empty($_POST['numprojet'])) {
    ...
    if (!empty($_POST['numprojet'])) {
        ...
        $req = "SELECT idprojet, titre, datedebutprojet, numero, dateprojet FROM concerne, projet WHERE idprojet_projet = idprojet and lower(numero) like lower(?) and idcentrale_centrale=? and idstatutprojet_statutprojet=? AND trashed =FALSE "
                . "union "
