...
class FPDF {
    ...
    function Header() {
        ...
        $this->WriteHTML($this->Image("styles/img/logo-renatech.jpg", 5, 10, 220));
        $this->WriteHTML('<u>');
        $this->Text(95, 20, TXT_TITREDOCUMENT0);
        $this->Text(95, 27, TXT_TITREDOCUMENT1 . ':');
        if(!empty($slibellecentrale)){
            if(strlen($slibellecentrale)<10){//STRING de taille <10
                $this->Text(165, 27, $slibellecentrale);
            }else{
                $this->Text(95, 34, $slibellecentrale);
            }
        }
        $this->WriteHTML('</u>');
        // Saut de ligne
