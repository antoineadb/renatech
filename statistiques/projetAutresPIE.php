...
if (IDTYPEUSER == ADMINNATIONNAL && !isset($_GET['anneeType'])) {
    ...
    $nbtotalprojet = $manager->getSinglebyArray("SELECT count(distinct idprojet) FROM projet,concerne where idprojet_projet = idprojet and idstatutprojet_statutprojet!=? and idstatutprojet_statutprojet!=? "
            . "and idstatutprojet_statutprojet!=? and extract(year from dateprojet)>2012 and extract(year from dateprojet)<=? ", array(ACCEPTE, REFUSE, ENATTENTEPHASE2, (date('Y') - 1)));
    ...
    $nbprojetExogeneCollaboratif = $manager->getSinglebyArray("SELECT count(distinct co.idprojet_projet) FROM  projet p, projetpartenaire pr, utilisateur u, creer c,concerne co WHERE p.idprojet = c.idprojet_projet 
        ...
        and idstatutprojet_statutprojet!=? and idstatutprojet_statutprojet!=? and extract(year from dateprojet)>2012 and extract(year from dateprojet)<=?", array(ACCEPTE, REFUSE, ENATTENTEPHASE2, (date('Y') - 1)));
    ...
    $nbprojetExogeneCollaboratif = $manager->getSinglebyArray("SELECT count(distinct co.idprojet_projet) FROM  projet p, projetpartenaire pr, utilisateur u, creer c,concerne co WHERE p.idprojet = c.idprojet_projet 
        AND pr.idprojet_projet = p.idprojet AND c.idutilisateur_utilisateur = u.idutilisateur and  co.idprojet_projet = p.idprojet And u.idcentrale_centrale IS NOT NULL and idstatutprojet_statutprojet!=?  
        and idstatutprojet_statutprojet!=? and idstatutprojet_statutprojet!=? and extract(year from dateprojet)>2012 and extract(year from dateprojet)<=?", array(ACCEPTE, REFUSE, ENATTENTEPHASE2, $_GET['anneeType']));
    $nbprojetInterne = $nbtotalprojet - $nbprojetExogeneExterne - $nbprojetExogeneCollaboratif;
    $string0 = '["' . TXT_PROJETINTERNE . '",' . $nbprojetInterne . '],';
    $string3 = '["' . TXT_PROJETEXOEXTERNE . '",' . $nbprojetExogeneExterne . '],';
    $string4 = '["' . TXT_PROJETEXOCOLLABORATIF . '",' . $nbprojetExogeneCollaboratif . '],';
...
if (IDTYPEUSER == ADMINLOCAL) {
    ...
    . "  and idstatutprojet_statutprojet!=? and co.idcentrale_centrale=? and extract(year from dateprojet)>2012 and extract(year from dateprojet)<=?",array(ACCEPTE, REFUSE, ENATTENTEPHASE2, IDCENTRALEUSER,(date('Y')-1)));
    $title = TXT_PROJETPARDATETYPEPOURANNEE . (date('Y')-1);
    $subtitle = TXT_NBPROJET . ': ' . $nbtotalprojet;
    $nbprojetExogeneExterne = $manager->getSinglebyArray("SELECT count(distinct p.idprojet) FROM   creer cr,   projet p,utilisateur u,concerne co WHERE   cr.idprojet_projet = p.idprojet 
            AND  u.idutilisateur = cr.idutilisateur_utilisateur AND  u.idcentrale_centrale is null and  co.idprojet_projet = p.idprojet and extract(year from dateprojet)>2012 and idstatutprojet_statutprojet!=? 
            and idstatutprojet_statutprojet!=?  and idstatutprojet_statutprojet!=? and p.idprojet not in(select idprojet_projet from projetpartenaire ) and co.idcentrale_centrale=? and extract(year from dateprojet)<=?", 
            array(ACCEPTE, REFUSE, ENATTENTEPHASE2, IDCENTRALEUSER,(date('Y')-1)));
    $nbprojetExogeneCollaboratif = $manager->getSinglebyArray("SELECT count(distinct co.idprojet_projet) FROM  projet p, projetpartenaire pr, utilisateur u, creer c,concerne co WHERE p.idprojet = c.idprojet_projet 
        AND pr.idprojet_projet = p.idprojet AND c.idutilisateur_utilisateur = u.idutilisateur and  co.idprojet_projet = p.idprojet And u.idcentrale_centrale IS NOT NULL and idstatutprojet_statutprojet!=?
        and idstatutprojet_statutprojet!=? and idstatutprojet_statutprojet!=? and extract(year from dateprojet)>2012 and co.idcentrale_centrale=? and extract(year from dateprojet)<=?",
            array(ACCEPTE, REFUSE, ENATTENTEPHASE2, IDCENTRALEUSER,(date('Y')-1)));
    $nbprojetInterne = $nbtotalprojet - ($nbprojetExogeneExterne + $nbprojetExogeneCollaboratif);
    $string0 = '["' . TXT_PROJETINTERNE . '",' . $nbprojetInterne . '],';
    $string3 = '["' . TXT_PROJETEXOEXTERNE . '",' . $nbprojetExogeneExterne . '],';
