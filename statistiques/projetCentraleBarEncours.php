...
if (IDTYPEUSER == ADMINNATIONNAL) {
    ...
    foreach ($centrales as $key => $centrale) {
            $nbProjet = $manager->getSinglebyArray(
                    ...
                    AND idcentrale!=?",array($centrale[1],TRUE,ENCOURSREALISATION,IDCENTRALEAUTRE));
...
if (IDTYPEUSER == ADMINLOCAL) {
    ...
    foreach ($years as $key => $year) {
            $nbProjet = $manager->getSinglebyArray("
                    ...
                    AND  trashed != ? 
                    AND idstatutprojet_statutprojet=?", array(IDCENTRALEUSER,$year[0],TRUE,ENCOURSREALISATION));            
