...
if (IDTYPEUSER == ADMINNATIONNAL && isset($_GET['anneeProjetEncours'])) {  
    $nbtotalprojet = $manager->getSinglebyArray("select count(distinct idprojet) from  projet,concerne where idprojet_projet=idprojet and extract(year from dateprojet)<=? AND idcentrale_centrale is not null "
            . "and  trashed != ?  and idstatutprojet_statutprojet=? and extract(year from dateprojet)>2012",array($_GET['anneeProjetEncours'],TRUE,ENCOURSREALISATION));
    for ($i = 0; $i < count($arraylibellecentrale); $i++) {
        $donneeProjet = $manager->getSinglebyArray("SELECT count(idprojet) FROM projet,concerne WHERE idprojet_projet = idprojet AND idcentrale_centrale=?  and extract(year from dateprojet)<=? and  trashed != ?  "
                . "and idstatutprojet_statutprojet=? and extract(year from dateprojet)>2012",array($arraylibellecentrale[$i]['idcentrale'],$_GET['anneeProjetEncours'],TRUE,ENCOURSREALISATION));        
        if ($nbtotalprojet != 0) {
    ...
    $nbtotalprojet = $manager->getSinglebyArray("select count(distinct idprojet) from  projet,concerne where idprojet_projet=idprojet and extract(year from dateprojet)<=? AND idcentrale_centrale is not null "
            . "and  trashed != ?  and idstatutprojet_statutprojet=? and extract(year from dateprojet)>2012",array(date('Y')-1,TRUE,ENCOURSREALISATION));
    for ($i = 0; $i < count($arraylibellecentrale); $i++) {
        $donneeProjet = $manager->getSinglebyArray("SELECT count(idprojet) FROM projet,concerne WHERE idprojet_projet = idprojet AND idcentrale_centrale=? and extract(year from dateprojet)>2012"
                . "and extract(year from dateprojet)<=? and  trashed != ?  and idstatutprojet_statutprojet=? ",array($arraylibellecentrale[$i]['idcentrale'],(date('Y')-1),TRUE,ENCOURSREALISATION));        
        if ($nbtotalprojet != 0) {
            $string0.='["' . $arraylibellecentrale[$i]['libellecentrale'] . '",' . $donneeProjet . '],';
...
if (IDTYPEUSER == ADMINLOCAL) {   
    $nbtotalprojet = $manager->getSinglebyArray("SELECT count(idprojet) FROM projet,concerne WHERE idprojet_projet=idprojet and idcentrale_centrale=? and extract(year from dateprojet)>2012 "
            . "and extract(year from dateprojet)<=?",array(IDCENTRALEUSER,(date('Y')-1)));
    for ($i = 0; $i < count($arraystatutprojet); $i++) {
        $donneeProjet = $manager->getSinglebyArray("SELECT count(idprojet) FROM projet,centrale,concerne WHERE idcentrale_centrale = idcentrale AND idprojet_projet = idprojet AND idcentrale_centrale=? 
            and idstatutprojet_statutprojet=? and extract(year from dateprojet)<=? and extract(year from dateprojet)>2012", array(IDCENTRALEUSER, $arraystatutprojet[$i]['idstatutprojet'],(date('Y')-1)));
        if ($nbtotalprojet != 0) {
            if ($lang == 'fr') {
                if($donneeProjet!=0){
