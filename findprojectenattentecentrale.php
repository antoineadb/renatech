...
$row = $manager->getListbyArray("select p.idprojet,p.dateprojet,p.numero,p.acronyme,p.titre,co.idstatutprojet_statutprojet from projet p,concerne co where co.idprojet_projet = p.idprojet and co.idcentrale_centrale=? and co.idstatutprojet_statutprojet=?", array($idcentrale, ENATTENTE));
        ...
        require([
            ...
            request.get("<?php echo '/'.REPERTOIRE; ?>/tmp/projetsEnattentecentrale.json", {
                ...
                function deleteProjet(enleve, idx) {
                    ...
                    return '<button class="btnSuppr"  onClick="rep.show();">Supprime</button>';
                }
...
<div data-dojo-type="dijit/Dialog" data-dojo-id="confirm" title="<?php echo TXT_CONFIRMATIONDELPROJET; ?>" data-dojo-props="iconClass:'dijitEditorIcon dijitEditorIconCut', showLabel: true"
     style="width: 380px;margin-left: 20px;   ">
    <table class="dijitDialogPaneContentArea" >
        <tr>
            <td><button type="submit" data-dojo-type="dijit/form/Button"   id="submitYes" onclick="window.location.replace('<?php echo '/'.REPERTOIRE?>/modifBase/deleteprojetenattente.php?lang=<?php echo $lang;?>&idprojet='+document.getElementById('results').value+'')">
                    <?php echo TXT_OUI; ?></button></td>
            <td><button type="submit" data-dojo-type="dijit/form/Button"  id="submitNo" data-dojo-props="onClick:function(){confirm.hide();}
                        ">
                    <?php echo TXT_NON; ?></button></td>
            <td><button data-dojo-type="dijit/form/Button" type="submit" data-dojo-props="onClick:function(){confirm.hide();}" id="efface"><?php echo TXT_ANNULE; ?></button></td>
        </tr>
    </table>
</div>
<?php
